...
const Orders = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black gray-particles flex items-center justify-center">
        <div className="w-16 h-16 bg-gold-500/20 rounded-full flex items-center justify-center ">
          <div className="spinner w-8 h-8 text-gray-400"></div>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-black gray-particles">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-12 text-center">
          ...
          <p className="text-gray-400 text-xl">Revisa el estado de tus pedidos y el historial de compras</p>
        </div>
        ...
        {orders.length === 0 ? (
          <Card className="elegant-card ">
            <div className="text-center py-16">
              ...
              <p className="text-gray-400 text-lg mb-8">Comienza a comprar para ver tus pedidos aquí.</p>
              <Link to="/products">
          ...
          <div className="space-y-8">
            {orders.map((order) => {
              ...
              return (
                <Card key={order.id} className="elegant-card  hover:shadow-2xl hover:shadow-gold-500/20 transition-all duration-500">
                  {/* Header del pedido */}
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-8 pb-6 border-b border-gold-500/30">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-2xl font-bold text-gray-400">${order.total.toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6 mb-8">
                    {order.items.map((item) => (
                      <div key={item.id} className="flex items-center gap-6 p-4 bg-gray-800 rounded-lg">
                        <img
                          src={item.image}
                          alt={item.name}
                        ...
                        <span className="font-bold text-gray-400 text-xl">${item.price.toFixed(2)}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-gold-500/30">
                    <Button variant="outline" className="flex-1 sm:flex-none border-gold-500 text-gray-400 hover:bg-gold-500 hover:text-black transition-all duration-300">
                      Ver detalles
                    </Button>
