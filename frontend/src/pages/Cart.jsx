      ...
      <div className="min-h-screen bg-black gray-particles">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center">
            ...
            <p className="text-gray-400 text-lg mb-8">Parece que aún no has agregado productos a tu carrito.</p>
            <Link to="/products">
    ...
    <div className="min-h-screen bg-black gray-particles">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-lg">Revisa tus productos antes de finalizar la compra</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="space-y-6">
              {cartItems.map((item) => (
                <Card key={item.id} className="elegant-card hover:shadow-2xl hover:shadow-gray-500/20 transition-all duration-500 group hover:scale-105  overflow-hidden">
                  <div className="flex items-center gap-6 p-6">
                    ...
                    <div className="w-32 h-32 flex-shrink-0 relative">
                      ...
                      />
                      <div className="absolute top-2 right-2 bg-gray-500 text-black px-2 py-1 rounded-full text-xs font-bold">
                        {item.quantity}x
                      </div>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-gray-300 text-sm mb-3">Stock disponible: {item.stock}</p>
                      <p className="text-gray-400 text-lg font-medium">${item.price} c/u</p>
                    </div>

                    {/* Cantidad */}
                    <div className="flex items-center border border-gray-500/30 rounded-lg bg-gray-800">
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity - 1)}
                        className="px-4 py-3 hover:bg-gray-500/20 text-gray-400 transition-colors rounded-l-lg"
                        disabled={item.quantity <= 1}
                      >
                        <Minus className="w-5 h-5" />
                      </button>
                      <span className="px-4 py-3 border-x border-gray-500/30 min-w-[4rem] text-center text-white font-bold text-lg">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                        className="px-4 py-3 hover:bg-gray-500/20 text-gray-400 transition-colors rounded-r-lg"
                    ...
                    <div className="text-right min-w-[8rem]">
                      <p className="font-bold text-gray-400 text-2xl">${(item.price * item.quantity).toFixed(2)}</p>
                    ...
                    <button
                      ...
                      className="p-4 text-gray-400 hover:text-red-400 hover:bg-red-500/10 transition-all duration-300 rounded-lg"
                    >
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="border-t border-gray-500/30 pt-4">
                  <div className="flex justify-between">
                    ...
                    <span className="text-xl font-bold text-gray-400">${totalWithTax.toFixed(2)}</span>
                  </div>
